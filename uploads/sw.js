/* ============================================================
   ...
   ============================================================ */
var CACHE = 'smartplan-v3';

// Файлы, которые всегда пытаются загрузиться с сети (обновляемые)
var FRESH = [
  './', './index.html', './app.js', './config.js', './data.js',
  './users_db.js', './areas_db.js', './work_db.js', './objects_db.js',
  './workers_db.js', './tasks_db.js', './db.js'
];
// Предзагрузка при установке (чтобы офлайн-режим работал сразу)
var PRECACHE = FRESH.concat(['./logo.png', './fon.webp', './icon-192.png', './manifest.json']);
...
function isFreshPath(pathname) {
  for (var i = 0; i < FRESH.length; i++) {
    ...
    if (f.slice(0, 2) === './' && f.slice(1) === pathname) return true; // './app.js' ↔ '/app.js'
    if (f === './' && pathname === '/') return true;
  }
  return false;
}
